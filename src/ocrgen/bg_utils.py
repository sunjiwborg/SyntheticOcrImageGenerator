...
def generate_background(bg_type_arg, is_random, size, bg_dir):
    bg_type = bg_type_arg
    if is_random:
    ...
    if bg_type == '1': # Gaussian Noise
        ...
        noise = np.random.normal(80, 120, array.shape)
        ...
        if not bg_dir or not os.path.exists(bg_dir):
            print(f"Warning: Background directory '{bg_dir}' not found. Falling back to white.")
            return Image.new('RGB', size, (255, 255, 255))
            
        bg_files = glob.glob(os.path.join(bg_dir, "*.*"))
        if bg_files:
            with Image.open(random.choice(bg_files)) as img:
                # ImageOps.fit resizes and crops to fill the 'size' WITHOUT squeezing
